...
function calendar(tYear,tMonth){ //달력 함수
    ...
    var row=1;  // 격주로 전체 휴관일, 자료실 휴관일을 설정해주는 변수
    ...
    var eDate= new Date();       // 변경된 날짜 객체 선언
    ...
    var calendarStr  = "<TABLE width = 333 height = 236>"
    ...
    for (var i=1; i<=lastDay; i++){       // 해당 월의 달력
        if(eDate.getFullYear()==nYear&&eDate.getMonth()==nMonth&&i==nDate){//오늘이면 today 스타일로 표시
            ...
            }else if(1<=col&&col<=4) {//그외 평범한 날이면
                calendarStr += "<TD class=workday>" + i + "</TD>"
            }
            else if(col==5){        // 쉬는 날이면
                if(row%2 == 1){
                    calendarStr += "<TD class=fullOffday>" + i + "</TD>"
                }
                else {
                    calendarStr += "<TD class=partOffday>" + i + "</TD>"
                }
            }
            else if(col==6){        //토요일이면
